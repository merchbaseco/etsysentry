...
const assertSyncState = (value: string | undefined): string | undefined => {
    if (!value) {
        return undefined;
    }

    if (!SYNC_STATE_VALUES.includes(value as (typeof SYNC_STATE_VALUES)[number])) {
        failWith({
            code: 'BAD_REQUEST',
...
export const parsePerformanceMode = (value: string | undefined): 'metrics' | 'table' => {
    if (!value) {
        return 'metrics';
    }

    if (!PERFORMANCE_MODE_VALUES.includes(value as (typeof PERFORMANCE_MODE_VALUES)[number])) {
        failWith({
            code: 'BAD_REQUEST',
...
export const parsePerformanceMetrics = (
    ...
    for (const item of parsed) {
        if (!METRIC_VALUES.includes(item as (typeof METRIC_VALUES)[number])) {
            failWith({
                code: 'BAD_REQUEST',
    ...
    return parsed as (typeof METRIC_VALUES)[number][];
};

export const filterKeywordItems = (items: KeywordItem[], flags: CliFlags): KeywordItem[] => {
...
export const filterListingItems = (items: ListingItem[], flags: CliFlags): ListingItem[] => {
    const trackingState = assertTrackingState({
    ...
    return items.filter((item) => {
        ...
        return (
            containsSearchText(item.title, flags.search) ||
...
export const filterShopItems = (items: ShopItem[], flags: CliFlags): ShopItem[] => {
    const trackingState = assertTrackingState({
        allowed: TRACKING_STATE_VALUES,
        value: flags.trackingState,
    });
